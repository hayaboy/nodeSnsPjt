...
// const bodyParser=require('body-parser'); //요청의 본문에 있는 데이터를 해석해서 req.body 객체로 만들어주는 미들웨어, 보통 폼 데이터나 AJAX 요청의 데이터를 처리 , 단 멀티파트(이미지, 동영상, 파일) 데이터는 처리하지 못함, 그 경우에는 multer 모듈을 사용하면 됨
const nunjucks=require('nunjucks'); // 퍼그의 HTML 문법 변화에 적응하기 분에게 힘든분에게 적합한 템플릿 엔진, HTML 문법을 그대로 사용, 자바스크립트 문법 사용, 파이썬의 템플릿 엔진인 Twig와 문법이 상당히 유사
...
var app = express();  // 익스프레스 내부에 http 모듈 내장되어 있으므로 서버로의 역할을 함
...
}); // watch:true이면 HTML 파일이 변경될 때 템플릿 엔진을 다시 렌더링
...
//app.use에 매개변수가 req, res, next인 함수를 넣으면 됩니다. 미들웨어는 위에서부터 아래로 순서대로 실행되면서
//요청과 응답 사이에 특별한 기능을 추가할 수 있습니다.
...
// error handler 에러 처리 미들웨어, 모든 매개변수를 사용하지 않더라도 매개변수가 반드시 네 개여야 합니다.
